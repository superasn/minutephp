...
##Project status##
It's highly experimental code! Right now, I just want to share the concept (and code) to see **if anyone interested in collaborating with me** :)

###Demo: Facebook like newsfeed (in two minutes)###
...
1. ***posts[user_id][]*** - will find all rows in `posts` table with `user_id` = `:user_id` from URL (the placeholder). What this will do is:
   ...
   5. By default all models have a read permission of *same\_user*, so /newsfeed/2 will return *permission denied error* (if the logged in user's user_id is 1).
